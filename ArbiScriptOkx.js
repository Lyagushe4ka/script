...
const blockWaiting = web3.eth.subscribe('newBlockHeaders', async function(error ,blockHeader) {
    ...
    if (parseInt(blockHeader.l1BlockNumber ?? '0') >= blockNumber) {
        ...
        const notZeroBalance = setInterval(async function checkBalance() {
            const accountInfo = await spotAPI.getSpotAccounts();
            console.log(accountInfo.find(item => item.currency === 'ARB').balance);
            
            // making trade order when token balance > 10
            if (accountInfo.find(item => item.currency === 'ARB').balance > 10) {
                const orderExecute = await spotAPI.spot.placeOrder(tradingPair, orderType, orderSide, quantity);
...
const notZeroBalance = setInterval(async function checkBalance() {
    const accountInfo = await spotAPI.getSpotAccounts();
    console.log(accountInfo.find(item => item.currency === 'USDT').balance);
    
    // making trade order when token balance > 10
    if (accountInfo.find(item => item.currency === 'USDT').balance > 10) {
        const orderExecute = await spotAPI.spot.placeOrder(tradingPair, orderType, orderSide, quantity);
